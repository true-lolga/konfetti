...
function wppano_main_options_page(){
	?>
	<script src="<?php echo WPPANO_PATH.'/js/settings.js'?>"></script>	
	<div class="wrap">
		...
		<p>Please, refer to the <a href="http://wp-pano.com" target="_blank">official page</a> for support.</p>
	...
	<div class="donate">
		...
		<div>
			...
			<input type="hidden" name="business" value="alexey@wp-pano.com">
...
function wppano_plugin_settings(){ 
	...
	//register_setting( 'setup_group', 'hs_styles' );
	...
	//add_settings_field('hs_styles', 'User hotspot styles', 'wppano_fill_hs_styles', 'style_setup_page', 'style_setup' );
...
function wppano_fill_target_container(){
	...
	?>
	<input type="text" name="wppano_target_container" value="<?php echo esc_attr( $val ); ?>" />
...
function wppano_fill_posttype(){
	...
	<?php foreach ( $post_types as $post_type ) {
		...
		<tr>
			<td style="padding: 5px;">
				<label style="margin-right: 5pt;"><input type="checkbox" class="input-posttype" name="wppano_posttype[type][<?php echo $post_type;?>]" value="<?php echo $post_type;?>"<?php checked( $post_type,  $val['type'][$post_type]);?>/><?php echo __($postTypeObject->labels->menu_name);?></label>
			</td>
			<td style="padding: 5px;">
				<select class="posttype_select posttype_<?php echo $post_type; ?>" disabled name="wppano_posttype[hs_style][<?php echo $post_type;?>]" val="<?php echo $val['hs_style'][$post_type]; ?>">
				<?php 
			...
			<td style="padding: 5px;">
				<select class="posttype_select posttype_<?php echo $post_type; ?>" disabled name="wppano_posttype[window][<?php echo $post_type;?>]" val="<?php echo $val['window'][$post_type]; ?>">
				<?php $files = glob( WPPANO_BASEDIR . '/templates/*.php' ); 
				foreach( $files as $file ) {
					$file = pathinfo( $file ); ?>
					<option value="<?php echo $file['filename']; ?>"><?php echo $file['filename']; ?></option>
				<?php } ?>		
	...
	<p>Edit the <b>xml/wp-pano.xml</b> file in the wp-pano plugin directory to add/modify Hotspots styles.</p>
<?php }
