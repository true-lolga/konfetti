		...
		success: function(response){
			if(response.type == "success") {
				...
				for (i = 0; i < response.hotspots.length; i++) {
					...
					krpano.set('cms.post_title', response.hotspots[i]['title']);
					krpano.set('cms.post_thumbnail', response.hotspots[i]['thumbnail']);
...
function wppano_open_post(post_id) {
	...
	//wppano_wrapper.css('visibility', 'hidden');
	jQuery.ajax({
		type : "POST",
		dataType : "html",
		url : ajax.url,
